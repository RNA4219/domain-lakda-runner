...
| QEG | package/schema 0.2.0、Git SHA `6e33d77399611016f64e81d7147580b05387185b` |
| LLM runtime | release candidateごとに`/props`から取得したbuildを固定し、取得不能を不合格とする |
| LLM model | release candidateごとに期待model IDとGGUF pathを明示する |
| model SHA-256 | release candidate入力で期待値を必須とし、実GGUF全bytesからstream計算する |
| LLM context | 8,192 tokens |
| LLM endpoint | `http://127.0.0.1:8080/v1` |

この表は対応可能な全環境を意味しない。runtime build、chat template、model ID、GGUF SHA-256はrelease candidateごとに実測値と期待値を照合し、過去の観測値をfallback既定値として使用しない。Linux、macOS、Arm、別runtime、別GGUF、別量子化はpost-v1の互換性評価対象とする。
...
### 5.5 v0.2 責務分離と逐次 worker

| ID | 強度 | 要件 |
|---|---|---|
| REQ-FN-013 | Must | Artifact Store、Artifact Policy、Outcome Policy、Collector、HATE Exporterを分離し、Collectorは証跡保存、Policyは検査、Outcome Policyは最終outcome決定、HATE ExporterはHATE/v1 manifest生成・検証だけを担当すること。 |
| REQ-FN-014 | Must | `workers=2..4`を逐次実行し、各workerを独立run ID、run directory、metadata、action sequence、HATE manifestとして保存し、基盤errorを含むworker失敗後も残りworkerを実行すること。batch outcomeは`error > failed > partial > passed`で集約すること。 |
| REQ-FN-015 | Must | `domSnapshots=true`では成功した各action後にredacted HTML snapshotを保存し、script本文、form値、password/token/secret要素、`data-lakda-sensitive`要素の内容および全属性をrawで保存しないこと。 |

### 5.6 v0.2 セキュリティと設定契約

| ID | 強度 | 要件 |
|---|---|---|
| REQ-SEC-008 | Must | text artifactを保存前redactionし、保存後実bytesをsecret/PII scanすること。残存検出はartifact削除と安全なfailure記録を行い`error`、容量超過は有効証跡なら`partial`とすること。 |
| REQ-NF-007 | Must | batch全体で共有する60秒sliding-window Action Budgetを持ち、上限到達時はLLM/Playwright操作を行わず`rate_limit`、`partial`でworkerを終了すること。top-level `seed`、`llm.seed`、`fixtureResetConfigured`は正規化し、矛盾設定を拒否すること。 |
### 5.7 v0.2.1 証跡 hardening と Policy 順序
...
`run outcome` は技術的なrun結果であり、QEG `gate verdict`とは別である。LakdaはHATE/v1 manifestまでを担当し、QEG変換とGate判定を行わない。

`fixtureResetConfigured` は `fixtureReset` の有無から導出し、`llm.seed` はtop-level `seed`へ同期する。明示値が導出値と矛盾するconfigは実行前に拒否する。

`llm-explore` は指定モデル不在時に `error/llm_error` とし、deterministic modeは `llm_status=unavailable` を保存して継続できる。Action Budget上限到達時は待機・自動再開せず、対象workerを `partial/rate_limit` で終了する。

`domSnapshots=true` のDOMはbrowser内cloneとhost redaction後のHTMLのみを保存し、容量超過は `partial/artifact_limit`、sanitization/保存失敗は `error/artifact_failure` とする。`trace`/`screenshot` はbrowserが実際に起動したnon-pass runにだけ適用する。
### 5.8 v0.2.1 実LLM証跡とRelease Gate

| ID | 強度 | 要件 |
|---|---|---|
| REQ-LLM-010 | Must | 規範`full` profileをworkers=1、通常20ケース×3回とcritical 10ケース×3回の合計90 child runsに固定し、`worker-smoke` profileをworkers=2、critical 10ケース×1回の20 child runsに固定すること。custom実行はAC-007/AC-010への適格性を持たないこと。 |
...
動的candidate再生成、状態fingerprint、遷移graph、coverage誘導探索、strict replay、
Airtest/PocoおよびSecurity adapterの追加要件は
[REQUIREMENTS-ADAPTIVE-EXPLORATION.md](REQUIREMENTS-ADAPTIVE-EXPLORATION.md)を正本ドラフトとする。
これらは現行v1のMustまたは既存modeの意味を変更せず、対応する仕様、schema、評価、
Task Seedを追加してからpost-v1実装対象へ昇格する。

post-v1の具体仕様、要件の一次所有、仕様書ごとの二段階チェックリストは
[適応型探索仕様書群](docs/spec/adaptive-exploration/)を正本とし、受入手順と必要証跡は
[適応型探索評価仕様](docs/spec/adaptive-exploration/EVALUATION-ADAPTIVE-EXPLORATION.md)を参照する。
...
| AC-007 | REQ-LLM-003、REQ-LLM-010 | 規範`full` profileの通常LLM decision 20件×3回（60 decision）が揃い、strict JSON Schema適合率100%であること。 |
...
| AC-010 | REQ-LLM-001〜011 | 規範`full` profileのcritical LLM decision 10件×3回（30 decision）が揃い、全件期待outcomeを満たすこと。 |
...
| AC-014 | REQ-FN-014、REQ-NF-007、REQ-LLM-010 | `workers=2..4`の逐次実行契約を決定的テストで満たし、規範`worker-smoke` 20 child runsを補助証跡として、workerごとのseed、独立run/HATE manifest、全worker継続を実機確認すること。worker-smokeはAC-007/AC-010を達成扱いにしない。 |
| AC-015 | REQ-NF-007 | batch共有Action Budgetが60秒sliding windowで動作し、上限到達後のLLM/Playwright操作が0件、`terminationReason=rate_limit`、`partial`、exit code 2となること。 |
| AC-016 | REQ-FN-013、REQ-FN-015、REQ-SEC-008 | redacted DOM snapshotがactionごとにHATE manifestへ`static`として登録され、script、form値、token、email、sensitive要素が保存されず、実bytes security scanとPolicyが反映されること。容量超過・browser未起動・snapshot保存失敗のtermination reasonも検証する。 |
...
v1 の文書上の完成は、`SPECIFICATION.md` の traceability matrix がすべての Must 要件と受入条件を参照し、孤立要件が0件であることを必要とする。実装のリリース判定は全MustとAC-001〜AC-020の証跡が揃った後に QEG が行う。Lakda 自身の `passed` はリリース `go` を意味しない。

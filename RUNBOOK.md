...
Task完了時は `docs/acceptance/AC-YYYYMMDD-xx.md` にcommit SHA、CI URL、dataset/model hash、実行結果を記録し、完了後に `docs/completion-record.md` とCHANGELOGへリンクする。Birdseye/Codemapはコード構成の生成後、workflow-cookbookの`--repo-root`指定でdry-run後に更新する。

...
- v0.2.1のArtifact Store、Artifact/Outcome Policy、共有Action Budget、逐次worker batch、HAR/security hardening、DOM容量判定。
- OpenAI 互換の loopback LLM endpoint と strict JSON decision 契約。HARはcontent=omitの一時captureをredactionしてから保存し、実bytes scanを通す。
...
- 出力の `outcome` は `passed | failed | partial | error`。termination reasonはartifact/executor/rate limitを保持し、QEGのgate verdictは出力しない。workers>1ではRunBatchResultと独立child runを返す。
...
| M5 | v0.2責務分離、Action Budget、逐次worker、DOM redaction | M4 |
| M6 | v0.2.1 HAR/security、Policy確定順序、DOM容量、異常系、受入 | M5 |
| M7 | post-v1 の cross-browser と追加 profile | M6 の受入後 |
...
- 次の実装入口は `docs/tasks/TASK.20260713-06.md` とする。

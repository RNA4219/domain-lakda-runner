...
- `LAKDA_ADAPTIVE_TARGET_MANIFEST`: `status=ready` の承認済みtarget manifest。通常caseは`lakda/target-manifest/v1`、`AC-AE-016`はEd25519署名、method/template scope、profile/capability/bridge digestを持つ`lakda/target-manifest/v2`を必須とする。origin、scope、mutation allowlist、action contract、settle policy、受入必須action IDの正本として扱う
...
`acceptanceId`は`AC-AE-001`から`AC-AE-016`まで、期待outcomeは`passed`、`failed`、`partial`、`error`のいずれかでなければなりません。各caseは`configDigest`によって正確なconfig bytesに束縛されます。runnerはoperatorによる上書きを受け入れず、hash済みcorpusからこれらの値とtarget revisionを導出します。revisionまたはconfig digestの不一致は、config読込みやtarget接続より前に拒否されます。corpus schemaは[adaptive-acceptance-corpus-v1.schema.json](../../schemas/adaptive-acceptance-corpus-v1.schema.json)です。通常reportは[adaptive-acceptance-case-v1.schema.json](../../schemas/adaptive-acceptance-case-v1.schema.json)、`AC-AE-016`はSecurity auditを必須にする[adaptive-acceptance-case-v2.schema.json](../../schemas/adaptive-acceptance-case-v2.schema.json)を使用します。
...
- `adaptive/acceptance-case-<caseId>.json`が通常caseでは`lakda/adaptive-acceptance-case/v1`、`AC-AE-016`では`lakda/adaptive-acceptance-case/v2`に適合し、HATE/v1 artifact refsを再利用し、必須case fieldをすべて含み、そのfile自体も再生成したHATE manifestに含まれている。
...
AC-AE-015では、承認済み実機証跡も必要です。AC-AE-016では、承認済み実security target、署名済みauthorization、policy/request counter/permit receipt/cleanup/kill switchを含むSecurity audit、明示的なhuman／oracle確認が必要であり、scannerまたはLLMの出力だけでは候補のままです。P7が`pending_external`から遷移できるのは、全16件のAC reportと後続のHATE／manual-bb／QEG証跡がreview・承認された後だけです。

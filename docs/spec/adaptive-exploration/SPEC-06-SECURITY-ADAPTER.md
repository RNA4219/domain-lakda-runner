...
| `owner` | 対象責任者または承認主体 |
| `targets` | allow host、path、HTTP method、request template SHA-256、target revision |
...
| `binding` | security profile、capability handshake、loopback bridge endpointのSHA-256 |
| `signature` | Ed25519、署名対象payload SHA-256、監査用signature ref |

欠落、期限切れ、target/environment/scope不一致、signatureまたはapproval ref不正の場合、active mutationとscan capabilityを無効にする。passive observationも認可を迂回せず、environment、host/path、method、request template、data policy、kill switchを満たす場合だけ許可する。各実行はauthorization、target revision、request ordinal、binding digestからpermit receiptを生成し、operator bridgeへ渡す。
...
production raceはv2では常に禁止する。productionはpassive-onlyとし、active raceを許可する将来契約は別schema revisionと明示的な再受入を必要とする。
...
- 実受入は署名済み`lakda/target-manifest/v2`と`lakda/adaptive-acceptance-case/v2`を使い、policy/request counter/permit receipt/cleanup/kill switchをSecurity auditとしてHATE/v1へ束縛する。

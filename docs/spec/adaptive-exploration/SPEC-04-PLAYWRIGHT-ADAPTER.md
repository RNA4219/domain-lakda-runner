...
同名role/nameまたはtest IDが重複するcontrolは、`row`、`listitem`、card、`dialog`の親scopeを用いて再解決する。scopeの識別子は一意な公開test ID、公開heading、policyで許可された非機微identifier hashの順に選び、親と子の双方が1件である場合だけscoped candidateにする。index、`nth()`、不安定CSSへのfallbackは禁止する。

各complete Observationの操作対象controlは、安全なcandidateまたは`lakda-coverage-debt/v1`の理由付きdebtへちょうど一度分類する。scopeを得られない同名control、無名icon、sensitive locatorはdebtとしてrole、公開nameまたはdigest、matched count、scope状態、target fingerprintだけを記録し、実行しない。

mutation分類はform method等のmechanical情報、product action contract、表示文字列heuristic、`unknown`の順に行う。根拠が競合またはunknownのcandidateは明示allowlistなしに実行しない。
...
`lightweight-dom/v1`は既定互換policyとして維持する。`consensus/v1`はDOM mutation、対象page/frameのscoped network、target topology、任意readinessの全signalがquiet/metであることを連続したstable windowで再確認してからsettledにする。deadline到達後に新たなsettled結果を返さず、timeout時は最後に観測したsignal状態と理由を返す。

network quiet除外は承認済みtarget manifestのpath prefixだけを許可し、host外またはprefix境界外のrequestへ拡張しない。

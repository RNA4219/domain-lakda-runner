...
Investigationは既存のlakda/investigation/v1を維持し、replayCount: 1、replayDigest、oracleRefs、evidenceRefsを記録する。必要時のtraceRef、configDigest、divergenceReason、terminationReasonも同schemaの後方互換な任意項目であり、refは絶対path、storageState、secret/PIIを含まないportableな値だけを許可する。
...
### P11 real acceptance / target manifest

P11 runnerは`lakda/target-manifest/v1`のpathを必須入力とし、manifest schema、status=ready、owner承認、HTTPS origin、allowHosts/pathPrefixes、target revision、config digest、mutation allowlist、reset/kill switch、settle policyをbrowserまたはbridge接続前に検証する。manifest fileのSHA-256とmanifest IDをcase reportへ保存し、corpus/configの宣言との不一致、path traversal、接続後のmanifest差し替えを拒否する。

新規reportはtargetごとのP0/P1 action IDと観測candidate auditを保持する。P0/P1候補欠落、ambiguous candidate、coverage debt、未分類mutation/controlがある場合はcaseをpassedにせず、理由付き`pending_external`またはfailureとして確定する。target manifestは実行許可そのものではなく、別途指定されたconfirmation、immutable corpus、case ID、target revision、外部承認がすべて必要である。fixture/mock manifestをreal evidenceへ昇格しない。

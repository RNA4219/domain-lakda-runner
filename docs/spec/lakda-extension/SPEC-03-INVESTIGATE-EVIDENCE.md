...
investigate開始時にLead、source run、trace、Signal、CombinationCase、artifactの存在、schema version、SHA-256を検証する。検証失敗は非0 exitで、再構築や別refへの暗黙置換をしない。

参照runをstrict replayし、pre/post fingerprint、settle、target topology、oracleを比較する。正常に再現した場合だけstep-by-step人手調査へ移行する。divergence、target lost、artifact欠落は人手調査へ進めず、理由付きrecordを確定する。
...
| command | 必須入力 | 出力 | 失敗 |
|---|---|---|---|
| lakda investigate | lead、source run | investigation record、replay evidence | ref/digest/divergence error |

...
観測済みの異常兆候をrule-firstでSignal化し、LLMを既存SignalのLead synthesisに限定して利用する。`rule-only`がconfigで明示設定されている場合に限り、LLM利用不能時に縮退運用できる契約を定義する。

## Scope

In: ExplorationSignal、ExplorationLead、rule-first抽出、loopback LLM、strict JSON、ref allowlist、scout evidence、明示設定時だけのrule-only degradation。
...
| REQ-LX-SIG-004〜006 | LLMは新Signalを作らず提示済みrefだけを束ねる。Leadはexploratory findingに留め、LLM停止時はconfigに明示設定された場合だけrule-onlyへ縮退し、それ以外はpartialで停止する。 |
...
| REQ-LX-LLM-006〜008 | providerの暗黙切替は禁止し、partialで停止する。configにrule-onlyが明示設定された場合だけrule-onlyへ縮退する。受理/拒否をJSONL証跡化し、Lead数hard capを設ける。 |
...
LLM timeout、schema不一致、attestation不一致、loopback外endpoint、token budget超過はpartialとして記録し、終了code 2を返す。Signal、rule-first Lead、run outcome、failure evidenceは失わない。LLMが使えないことをsuccessへ変換しない。

設定済みrule-only縮退は、configの `extensions.scouting.mode` が `rule-only` で、CLIが `--scout-mode=llm` を明示した場合だけ適用し、`effectiveMode=rule-only` とする。configが `loopback` のLLM失敗は `effectiveMode=none` とし、別providerへ切り替えない。純粋なrule-only実行だけが `scoutStatus=completed`、終了code 0となる。

### Evidence

受理・拒否判断は `accepted`、`responseSchemaHash`、`modelAttestationRef`、`inputDigest`、任意の `outputDigest`、`rejectionReasonDigest`、`runRevision`、`providerSwitchAttempted=false` をredaction済みJSONLへ記録する。拒否理由本文、raw prompt、raw responseは保存しない。

## Public I/O

| command | 必須入力 | 出力 | 失敗 |
|---|---|---|---|
| lakda scout | config、suite、scout config | signals、leads、scout log、report refs | schema/ref/attestation/timeoutはpartial、終了code 2 |
| lakda report leads | run directory | lead report JSON/HTML | unknown run/artifact、非0 |

scoutはconfigでrule-only modeを明示した場合に限り、LLMなしで実行できる。provider切替、Safety Policy変更、candidate生成はscoutの権限外とする。
...
4. 明示設定時だけのrule-only degradationとscout/report CLIを実装する。
...
- configで明示したrule-only modeでSignalとLead候補を生成し、LLM不在でも欠落しないこと。

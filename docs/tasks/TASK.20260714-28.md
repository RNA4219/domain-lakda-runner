...
# Task Seed: failure shrinkingの派生証跡
...
REQ-SHR-001〜005を、共通契約・Safety Policy・Artifact Policyを経由して最小実装とtests-firstで満たす。

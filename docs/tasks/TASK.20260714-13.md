...
# Task Seed: 観測state fingerprintの正規化
...
REQ-FP-001〜006を、共通契約・Safety Policy・Artifact Policyを経由して最小実装とtests-firstで満たす。

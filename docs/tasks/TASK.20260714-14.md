...
# Task Seed: DOM再観測candidate生成とstale拒否
...
REQ-ACT-001〜011を、共通契約・Safety Policy・Artifact Policyを経由して最小実装とtests-firstで満たす。

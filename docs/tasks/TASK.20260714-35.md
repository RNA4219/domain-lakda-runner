...
# Task Seed: real受入・HATE/QEG handoff
...
| linked checklist | [対応チェックリスト](../spec/adaptive-exploration/CHECKLIST-06-SECURITY-ADAPTER.md) |
| plan | [適応型探索実装計画](../IMPLEMENTATION-PLAN-ADAPTIVE-EXPLORATION.md) |

## Objective

AC-AE-015〜016を、共通契約・Safety Policy・Artifact Policyを経由して最小実装とtests-firstで満たす。
...
- Operator procedure, suite index verification, and evidence conditions: [P7 Real Adaptive Acceptance Runbook](../acceptance/P7-REAL-ACCEPTANCE-RUNBOOK.md).
- Lakda produces a revision/config-bound, schema-valid case verdict and a validated HATE/v1 manifest only. It does not produce manual-bb/QEG Gate verdicts, approvals, waivers, or records.
- AC-AE-015 requires approved real-device evidence. AC-AE-016 requires an approved real Security target, authorization/cleanup/kill-switch evidence, and explicit human or product-oracle confirmation.

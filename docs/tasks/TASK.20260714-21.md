...
# Task Seed: seeded generatorと未踏候補優先
...
REQ-EXP-001〜007を、共通契約・Safety Policy・Artifact Policyを経由して最小実装とtests-firstで満たす。

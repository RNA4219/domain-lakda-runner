...
# Task Seed: Airtest/Poco opt-in実機受入
...
AC-AE-015を、共通契約・Safety Policy・Artifact Policyを経由して最小実装とtests-firstで満たす。

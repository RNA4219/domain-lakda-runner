...
test("llm-explore consumes only supplied candidates and redacts decision secrets", async () => {
  ...
  try {
    ...
    expect(decisionPrompt).not.toContain('"path"'); expect(decisionPrompt).not.toContain('"selector"'); expect(decisionPrompt).not.toContain("fixture-secret"); expect(decisionPrompt).toContain('"chat_template_kwargs":{"enable_thinking":false}'); expect(decisionPrompt).toContain('"response_format":{"type":"json_object"}'); expect(decisionPrompt).toContain('"stream_options":{"include_usage":true}');
    const runDir = join(outputDir, result.runId.replace(/[^A-Za-z0-9._-]/g, "-"));
    const evidence = await readFile(join(runDir, "artifacts", "llm-decisions.jsonl"), "utf8");

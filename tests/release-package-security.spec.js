...
test("release package scanner verifies metadata, required files, hashes, and sensitive paths", async () => {
  ...
  const profilePath = root + ".release-profile.json";
  try {
    await mkdir(join(root, "dist"));
    await mkdir(join(root, "schemas"));
    await mkdir(join(root, "vendor", "hate", "v1"), { recursive: true });
    await writeFile(join(root, "package.json"), JSON.stringify({
      name: "domain-lakda-runner", version: "0.4.0-rc.2", private: true, main: "./dist/index.js", types: "./dist/index.d.ts",
    ...
    for (const [path, value] of [
      ...
      ["README.md", "# Lakda\n\n現在の候補版は `0.4.0-rc.2` です。\n"], ["RUNBOOK.md", "# Runbook\n"],
      ["dist/index.js", "export const version = '0.4.0-rc.2';\n"], ["dist/index.d.ts", "export declare const version: string;\n"],
    ...
    const output = root + ".scan.json";
    const stdout = execFileSync(process.execPath, [scanner, "--path=" + root, "--out=" + output], { encoding: "utf8" });
    expect(JSON.parse(stdout)).toMatchObject({ status: "passed", packageVersion: "0.4.0-rc.2", fileCount: 16 });
    expect(JSON.parse(await readFile(output, "utf8")).files.every(entry => /^[0-9a-f]{64}$/.test(entry.sha256))).toBe(true);

    await writeFile(profilePath, JSON.stringify({ schemaVersion: "lakda/release-profile/v1", releaseVersion: "9.9.9-rc.1" }));
    expect(() => execFileSync(process.execPath, [scanner, "--path=" + root, "--release-profile=" + profilePath], { encoding: "utf8", stdio: "pipe" })).toThrow();
    ...
    await rm(root + ".scan.json", { force: true });
  }
});

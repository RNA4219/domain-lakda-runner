...
import { assertLeadForReplay, buildScoutContext, groupLeadsRuleOnly, signalsFromTrace, scoutWithLoopback, writeScoutEvidence, type ExplorationLead } from "./adaptive/scouting.js";
import { assertInvestigation, assertPromotionReady, createInvestigation, promoteInvestigation, runStrictReplay, type Investigation } from "./adaptive/investigation.js";
import { buildReplaySteps, replayDetails, stableOracleRefs, validateAdaptiveReplayTrace, validateReplayScope, type AdaptiveReplayTrace } from "./adaptive/replay.js";
...
type ReplayRecord = { candidateId: string; execution: ExecutionResult | undefined; oracles: OracleResult[] };
type CompletedReplayRecord = { candidateId: string; execution: ExecutionResult; oracles: OracleResult[] };
...
function leadInput(value: unknown): LeadInput {
  ...
  if (current.schemaVersion === "lakda/exploration-lead/v1") return { lead: current as unknown as ExplorationLead };
  if (current.schemaVersion !== "lakda/lead-report-index/v1") throw new Error("unknown lead schemaVersion");
  const allowed = ["schemaVersion", "runId", "leadCount", "leads", "generatedAt", "signals", "leadObjects", "context"];
  ...
  if (!Array.isArray(current.leads) || !Array.isArray(current.leadObjects) || current.leadObjects.length === 0) throw new Error("lead report has no leadObjects");
  const selectedId = typeof current.leads[0] === "string" ? current.leads[0] : undefined;
  const selected = current.leadObjects.find(value => value && typeof value === "object" && (selectedId ? (value as Record<string, unknown>).leadId === selectedId : true));
  if (!selected) throw new Error("lead report selected lead is missing");
  return { lead: selected as ExplorationLead, ...(typeof current.runId === "string" ? { runId: current.runId } : {}) };
}
...
function replayRecords(trace: AdaptiveReplayTrace): CompletedReplayRecord[] {
  ...
  for (const entry of trace.trace) {
    if (entry.type === "candidate" && entry.candidate) {
      const next: ReplayRecord = { candidateId: entry.candidate.candidateId, execution: undefined, oracles: [] };
      current = next;
      records.push(next);
    } else if (entry.type === "execution" && current && current.execution === undefined && entry.executionResult) {
...
function expectedFailure(steps: ReturnType<typeof buildReplaySteps>): boolean {
  return steps.some(step => step.execution?.status !== "executed" || step.oracles.some(oracle => ["fail", "confirmed", "candidate"].includes(oracle.verdict)));
}

function actualFailure(records: CompletedReplayRecord[]): boolean {
  return records.some(record => record.execution.status !== "executed" || record.oracles.some(oracle => ["fail", "confirmed", "candidate"].includes(oracle.verdict)));
}

function portableRelativeRef(root: string, path: string): string {
  const value = relative(root, path).split("\\").join("/");
  if (!value || value.startsWith("/") || /^[A-Za-z]:/.test(value)) throw new Error("artifact ref is not portable");
  return value;
}
...
function runEvidenceRefs(runDir: string | undefined, outputPath: string): string[] {
  ...
  const required = ["adaptive/trace.json", "adaptive/oracle-results.jsonl", "exports/artifact-manifest.json"].map(ref => resolve(runDir, ref));
...
async function investigateCommand(flags: Flags): Promise<number> {
  ...
  if (config.mode !== "adaptive-explore" || !config.adaptive) throw new Error("investigate requires mode=adaptive-explore and adaptive config");
  ...
  validateReplayScope(traceInput, config.baseUrl, config.safety.allowHosts, config.adaptive.safety.allowTargetKinds);
  const investigation = createInvestigation(input.lead, reviewer);
  const out = resolve(stringFlag(flags, "out") ?? resolve(dirname(leadPath), investigation.investigationId + ".json"));
  ...
  const result = await runStrictReplay(investigation, async () => {
    ...
    try {
      ...
      if (!traceArtifact || !existsSync(traceArtifact)) throw new Error("replay trace artifact is missing");
      ...
      return { reproduced: false, inconclusive: true, evidenceRefs, traceRef: evidenceRefs[0], configDigest, terminationReason: run.terminationReason, details: { artifactFailure: true, outcome: run.outcome, terminationReason: run.terminationReason } };
    }
    const divergenceEntry = actualTrace.trace.find(entry => entry.type === "replay-divergence");
    const divergence = typeof divergenceEntry?.reason === "string" ? divergenceEntry.reason : undefined;
    const expected = expectedFailure(buildReplaySteps(traceInput));
    const actual = actualFailure(records);
    const inconclusive = !evidenceComplete || run.outcome === "error" || run.outcome === "partial" || (!divergence && run.terminationReason !== "completed" && run.outcome !== "failed");
    const effectiveDivergence = divergence ?? (!inconclusive && !expected && actual ? "unexpected-failure" : undefined);
    return {
  ...
  await writeCanonicalJson(out, result);
  console.log(JSON.stringify({ command: "investigate", out, status: result.status, replayCount: result.replayCount }, null, 2));
...
async function promoteCommand(flags: Flags): Promise<number> {
  ...
  if (kind !== "trace" && kind !== "suite") throw new Error("--kind must be trace or suite");
  ...
  assertPromotionReady(investigation, kind, refs, ref => {
    try { return statSync(resolve(root, ref)).isFile(); } catch { return false; }
  });
  const promotion = promoteInvestigation(investigation, kind, refs);
  const out = resolve(stringFlag(flags, "out") ?? resolve(dirname(investigationPath), promotion.promotionId + ".json"));
  await writeCanonicalJson(out, promotion);
  console.log(JSON.stringify({ command: "promote", out, promotionId: promotion.promotionId }, null, 2));
  return 0;
}
...
export async function runCli(argv: string[]): Promise<number> {
  try {
    ...
    if (command === "doctor") return await doctor(parsed.flags);
    if (command === "auth capture") return await capture(parsed.flags);
    if (command === "auth validate") return await validateAuth(parsed.flags);
    if (command === "combo gen") return await comboGen(parsed.flags);
    if (command === "combo verify") return await comboVerify(parsed.flags);
    if (command === "scout") return await scoutCommand(parsed.flags);
    if (command === "report leads") return await reportLeads(parsed.flags);

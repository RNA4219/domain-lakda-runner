...
export async function buildAndValidateManifest(
  ...
  provenance: ArtifactProvenance = { producerVersion: "0.3.0-rc.2", createdAt: "1970-01-01T00:00:00.000Z" },
...
export async function exportHate(runDir: string, out: string, securityByPath: Record<string, ArtifactSecurityRecord> = {}): Promise<object> {
  ...
  const manifest = await buildAndValidateManifest(runDir, metadata.runId, metadata.attempt, metadata.commitSha, metadata.artifactPolicy?.classification ?? "internal", { producerVersion: metadata.producerVersion ?? "0.3.0-rc.2", createdAt: metadata.endedAt ?? metadata.startedAt ?? "1970-01-01T00:00:00.000Z" }, policy.securityByPath, excluded, policy.verifiedArtifacts);

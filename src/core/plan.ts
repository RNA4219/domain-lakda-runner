...
export function createActionPlan(config: LakdaConfig, mode: RunMode = config.mode): ActionPlan {
  ...
  return { schemaVersion: "lakda/action-plan/v1", mode, seed: config.seed, baseUrl: new URL(config.baseUrl).toString(), actions };
}

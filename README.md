...
過去revision `600a037`の自己完結したQEG入力、`go` verdict、recordは[docs/release-gate/qeg-600a037/](docs/release-gate/qeg-600a037/gate-input.json)から辿れます。これはrc.2の承認ではありません。HATE export成功はQEG `go`と同義ではなく、QEG自身のschema-check、hash verify、Gate評価が必要です。
...
| P11 case受入 | 承認targetでcase単位のreal acceptanceを実行・検証 | `npm run acceptance:extension:real` | v2 runner/verifier実装済み。rc.2の実環境証跡は`pending_external` |

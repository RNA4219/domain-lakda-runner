...
| QEG | 0.2.0、Git `6e33d77399611016f64e81d7147580b05387185b` |
| llama.cpp | RCごとに`/props`から取得した実buildを固定し、取得不能を不合格とする |
| qwen4b | RC入力の期待model ID/GGUF SHA-256と実測値を照合し、固定fallbackを持たない |
...
```json
{
  ...
  "llm": {
    ...
    "modelPath": "C:\\models\\release-model.gguf",
    "runtimeEvidence": {
      "runtimeVersion": "attested-at-run-time",
      "runtimeBuild": "attested-at-run-time",
...
実LLM受入はpackage scriptで実行する。`npm run acceptance:real-llm:full`は`profile=full`を、`npm run acceptance:real-llm:worker-smoke`は`profile=worker-smoke`を固定する。従来optionを渡した実行は`custom`としてreportへ記録し、AC-007/AC-010の`eligible`を常にfalseとする。`npm run acceptance:verify -- --report <path> --bundle <path> --revision <sha>`はbundleを変更せず検証する。
...
2. 5秒以内にTCP接続できる。
3. `GET /v1/models` が成功し、実model IDが`expectedModelId`と完全一致する。
4. model file全bytesからstream計算したSHA-256がrelease入力の期待値と一致する。
5. `GET /props`からruntime build、build info、chat templateを取得でき、chat-template SHA-256を計算できる。
6. 32 tokens以下の`POST /v1/chat/completions`が20秒以内に成功し、response model IDが期待値と一致する。
...
Qwen 3.5系のdecision requestは `chat_template_kwargs.enable_thinking=false`、`response_format={"type":"json_object"}`、`stream_options.include_usage=true` を固定し、推論本文ではなく `message.content` のstrict JSONだけを判定対象とする。promptはaction用の `decision`、`candidateId`、`reason`、`confidence` の4キーを明示し、入れ子、extra key、URL、selector、code、path、commandを禁止する。実行時のchat template本体はloopback `/props` から取得してSHA-256を計算し、GGUF SHA-256・engine build・この固定kwargsとともに証跡へ保存する。
...
### 6.4 実LLM受入profileと証跡bundle

| profile | workers | corpus | repetition | child runs | 規範coverage |
|---|---:|---|---:|---:|---|
| `full` | 1 | 通常20＋critical 10 | 各3回 | 90 | AC-007通常60、AC-010 critical 30 |
| `worker-smoke` | 2 | critical 10 | 1回 | 20 | AC-014 supplementのみ |
| `custom` | option依存 | option依存 | option依存 | option依存 | AC-007/AC-010不適格 |
...
### 8.5 v0.2.1 Release Candidate Gate

release candidate専用workflowはself-hosted Windows/Qwen runnerで手動起動し、次の順序を固定する。

1. deterministic CI
2. 実LLM `full` 90-run
3. 実LLM `worker-smoke` 20-run
4. bundle verificationとsecurity scan
5. Code-to-gate strict（unresolved Critical/Highはblock、Mediumは根拠・担当・期限が必須）
6. 固定vendored HATE schemaと固定Git SHAのupstream schemaのbytes一致、全child manifest/hash検証
7. manual-bb staging evidence validation
8. QEG validate/gate/record

manual evidenceは対象revision、HTTPS staging originと明示allowlist、`testExecutionMode=real`、case結果、operator、時刻、artifact hashを持つ。credential値は保存しない。`testExecutionMode=mock`、full 90-run未達、worker bundle欠落、manual未実施、HATE upstream未確認、Code-to-gate strict未達のいずれかではQEG入力を生成せずreleaseを`hold`とする。Lakdaのprepare処理はQEG入力候補までを構成できるが、QEG recordとGate verdictはQEG processだけが生成する。
...
- QEG schema: `C:\Users\ryo-n\Codex_dev\quality-evidence-graph\schemas` at Git `6e33d77399611016f64e81d7147580b05387185b`
